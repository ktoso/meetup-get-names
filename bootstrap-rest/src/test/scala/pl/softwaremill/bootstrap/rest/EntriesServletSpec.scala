...
import pl.softwaremill.bootstrap.service.data.{EntriesWithTimeStamp, Member}
...
class EntriesServletSpec extends BootstrapServletSpec {
  ...
  def onServletWithMocks(test:(MeetupNamesSlurper, UserService) => MatchResult[Any]): MatchResult[Any] = {
    val userService = mock[UserService]

    val entryService = mock[MeetupNamesSlurper]
    entryService.count() returns 4
    val entryOne: Member = Member("1", "<script>alert('hacker')</script>", "Jas Kowalski", "")
    ...
    val servlet: SlurpServlet = new SlurpServlet(entryService, userService)
    addServlet(servlet, "/*")
